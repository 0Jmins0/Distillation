...
parser.add_argument("--num_epochs", type=int, default=40,help="Number of epochs to train (default: 1)")
...
if args.model_name == "MV_AlexNet_dis" or args.model_name == "MV_AlexNet_dis_Pre":
    ...
    # model_T.load_state_dict(torch.load(f"../models/train_models/{args.train_data}/MVCNN_CLIP_L/epochs_14_lr_{args.lr}_batch_{args.batch_size}.pth")['model_state_dict'])
    model_T.eval()
...
if args.model_name == "MVCNN_CLIP":
    ...
    model = MV_AlexNet(num_views = args.num_views, is_dis = True, batch_size = args.batch_size, PretrainedModel_dir = Pretrained_model_path).to(device)
    init_lr = args.lr * 0.1
    optimizer = optim.Adam([
    {"params": model.features.features.parameters(), "lr": init_lr},
    {"params": model.retrieval.parameters(), "lr": init_lr}   # 新增层高学习率
...
if args.model_name == "MV_AlexNet_dis_Pre":
    if args.loss == "SimpleFeatureDistillationLoss":
        log_dir = f"../output/tensorboard_logs/exp/{args.train_data}/{args.model_name}/{args.loss}/lr_{args.lr}_batch_{args.batch_size}"
    else:
        log_dir = f"../output/tensorboard_logs/exp/{args.train_data}/{args.model_name}/{args.loss}/lr_{args.lr}_batch_{args.batch_size}_rGL_{args.rGL}_rOI_{args.rOI}"
else:
...
for epoch in range(start_epoch + 1, num_epochs):
    ...
    # validation_loss = 0.0
    # with torch.no_grad():
    #     for anchor_images, positive_images, negative_images in validation_loader:
    #         anchor_images = anchor_images.view(-1, *anchor_images.shape[-3:])
    #         positive_images = positive_images.view(-1, *positive_images.shape[-3:])
    #         negative_images = negative_images.view(-1, *negative_images.shape[-3:])
    ...
    if args.model_name == "MV_AlexNet_dis_Pre":
        if args.loss == "SimpleFeatureDistillationLoss":
            save_path = f"../models/exp/train_models/{args.train_data}/{args.model_name}/{args.loss}/best_model_lr_{args.lr}_batch_{args.batch_size}.pth"
        else:
            save_path = f"../models/exp/train_models/{args.train_data}/{args.model_name}/{args.loss}/best_model_lr_{args.lr}_batch_{args.batch_size}_rGL_{args.rGL}_rOI_{args.rOI}.pth"
    else:
